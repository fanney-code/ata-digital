...
export interface MasterScholarItem {
  id: string;
  name: string;
  permanentUid: string;
  email: string;
...
export const INITIAL_MASTER_SCHOLARS: MasterScholarItem[] = [];
...
export const MasterStudentDirectoryView: React.FC<MasterStudentDirectoryViewProps> = ({
  ...
  const [selectedScholarDossier, setSelectedScholarDossier] = useState<MasterScholarItem | null>(null);

  // Map database students directly into authoritative scholar items
  const combinedScholars = useMemo<MasterScholarItem[]>(() => {
    ...
    return students.map((s) => {
      ...
      const degreeLevel: MasterScholarItem['degreeLevel'] = isDoc
        ? 'Doctoral'
        : progName.toLowerCase().includes('master') || progName.toLowerCase().includes('m.th') || progName.toLowerCase().includes('m.div')
        ? 'Masters'
        : 'Bachelors';
      ...
      let governanceStatus: MasterScholarItem['governanceStatus'] = 'ACTIVE_SCHOLAR';
      ...
      return {
        ...
        institutionSubtext: `${instName} • Regional Node`,
        activeProgram: progName,
        programSubtext: `${primaryReg?.department?.name || 'Faculty of Theology'} • Intake ${primaryReg?.academic_year?.slice(0, 4) || '2026'}`,
        degreeLevel,
  ...
  const filteredScholars = useMemo(() => {
    return combinedScholars.filter((scholar) => {
      ...
      // Seminary filter
      if (selectedSeminary !== 'ALL' && !scholar.institutionName.toLowerCase().includes(selectedSeminary.toLowerCase())) {
        return false;
      }
      ...
      // Tab filter
      if (activeTab === 'ACTIVE' && scholar.enrollmentStatus !== 'Active Enrolled' && scholar.enrollmentStatus !== 'Dual Enrolled') {
        return false;
      }
  ...
  const handleExportRoster = () => {
    ...
    link.setAttribute('download', `ATA_Master_Student_Roster_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs border-b border-slate-200/80 pb-3">
        <div className="flex items-center gap-2 text-slate-500 font-medium tracking-wide">
          <span className="hover:text-slate-800 transition-colors cursor-pointer">ATA Central Governance</span>
          <span>&rsaquo;</span>
          <span className="text-slate-900 font-bold">Student Directory &amp; Master Roster</span>
        </div>
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#dcfce7]/70 text-[#006f67] border border-[#86efac]/60 font-semibold shadow-2xs">
          ...
          <span>Asia-Wide Accredited Scholar Registry • Real-time Sync</span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 max-w-2xl leading-relaxed">
            Centralized repository of all matriculated theological scholars across 142 accredited ATA member
            institutions. Conduct cross-institutional audits, verify conferred credentials, and inspect lifetime
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
          ...
          >
            ...
            <span>Export Student Roster (.csv / .xlsx)</span>
          </button>
          ...
          >
            ...
            <span>Batch Audit Verification</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-2xs space-y-3">
        <div className="flex flex-col md:flex-row items-center gap-3">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              placeholder="Search by Candidate Name, Permanent UID (STU-2026-XXXX), Email, or Institution..."
        ...
        <div className="flex items-center gap-2 overflow-x-auto pt-2 border-t border-slate-100 no-scrollbar text-xs">
          ...
          >
            ...
            <span className="opacity-80 font-normal">42</span>
          </button>
        </div>
      </div>

      {/* 5. Master Student Directory Table */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-3.5 bg-slate-50/50 border-t border-slate-100 text-xs text-slate-500">
          <div className="flex items-center gap-3">
            <p>
              ...
              {Math.min(currentPage * pageSize, filteredScholars.length)} of {filteredScholars.length} scholars
            </p>
            <span>|</span>
      ...
      {/* 6. Accreditation Statute Protocol Directive Card */}
      ...
      {/* MODAL 1: Batch Audit Verification Modal */}
      {batchAuditModalOpen && (
      ...
      {/* MODAL 2: Inspect Governance Code Modal */}
      ...
      {/* MODAL 3: Scholar Dossier & Lifetime Milestones Drawer */}

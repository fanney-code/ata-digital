...
export const RegistrarDashboard: React.FC<RegistrarDashboardProps> = ({
  ...
  const registrarName = toTitleCase(user?.full_name || 'Registrar');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs flex flex-col justify-between">
          ...
          <div className="flex flex-wrap items-center gap-1.5 mt-4 text-xs font-semibold text-emerald-600">
            <TrendingUp className="h-3.5 w-3.5 shrink-0" />
            <span className="whitespace-nowrap">{totalEnrolled > 0 ? `${totalEnrolled} active` : 'Active'}</span>
            <span className="text-slate-400 font-normal whitespace-nowrap">candidates registered in database</span>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 shadow-xs space-y-4">
        ...
        {/* Column Headers */}
        <div className="hidden lg:grid grid-cols-12 gap-4 px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 bg-slate-50/60 rounded-xl">
            ...
            {paginatedList.map((reg) => {
              ...
              return (
                <div
                  key={reg.id}
                  onClick={() => onSelectRegistration(reg)}
                  className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center p-3.5 sm:p-4 rounded-xl border border-slate-200/90 hover:border-slate-300 bg-white hover:bg-slate-50/50 transition-all shadow-2xs cursor-pointer group"
                >
                  {/* Reg ID */}
                  ...
                  <div className="lg:col-span-2 flex items-center justify-start">
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold whitespace-nowrap ${badgeCfg.style}`}
                    >
                      {badgeCfg.label}
                    </span>
                  </div>

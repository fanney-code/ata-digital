        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {students.map((st) => (
              <div
                key={st.id}
                className="p-5 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm space-y-3"
              >
                ...
                <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-400">
                  ...
                  {st.date_of_birth && (
                    <div className="flex items-center gap-2">
                      ...
                      <span>DOB: {st.date_of_birth}</span>
                    </div>
                  )}
                </div>
              </div>
